...
/* Hold copy of status register state here for rPOLL_TX_TO_RESP_RX_DLY_UUSeference so that it can be examined at a debug breakpoint. */
...
int dw_main(void)
{
    ...
    terminal_print(APP_NAME, strlen(APP_NAME));
    ...
    {
        terminal_print("INIT FAILED", strlen("INIT FAILED"));
        while (1)
        { };
    }
